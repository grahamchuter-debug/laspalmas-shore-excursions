...
import { getCruisePlanningPageBySlug, getAllCruisePlanningSlugs } from "@/data/cruise-planning";
import type { AttractionPage, GuidePage } from "@/data/types";

export type GuideEntry =
  | { kind: "highlight"; page: AttractionPage }
  | { kind: "experience"; page: GuidePage }
  | { kind: "cruise-planning"; page: GuidePage };
...
export function getGuideBySlug(slug: string): GuideEntry | undefined {
  ...
  const cruisePlanning = getCruisePlanningPageBySlug(slug);
  if (cruisePlanning) return { kind: "cruise-planning", page: cruisePlanning };
  return undefined;
}

export function getAllGuideSlugs(): string[] {
  return [...getAllHighlightSlugs(), ...getAllExperienceSlugs(), ...getAllCruisePlanningSlugs()];
}
